...
<ipython-input-3-8a779299c426> in <module>()
----> 1 lucky_dice(3)

<ipython-input-2-7c4948292301> in lucky_dice(number)
      4         print(outcomes[1])
      5     if number < 6:
----> 6         print(outcomes[2])
      7 
...
2.  The second shows some code in the function `lucky_dice`,
    with an arrow pointing to Line 6 (which is `print(outcomes[2])`).
...
the program encountered an error on Line 6, when it tried to run the code `print(outcomes[2])`.
