...
:   A remark in a program that is intended to help human readers understand what is going on,
    ...
    Comments in Python, R, and the Unix shell start with a `#` character and run to the end 
    of the line;
...
:   The practice of writing programs that check their own operation to catch errors 
...
parameter
:   A variable named in a function's declaration that is used to hold a value passed to a call.
...
precondition
:   A condition that must be true for a function (or other block of code) to run correctly.

regression
:   To re-introduce a bug that was once fixed.

return statement
:   Causes a function to stop executing and return a value to its caller immediately.
